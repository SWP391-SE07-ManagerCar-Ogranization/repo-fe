...
export default function Chatbox({ group, role }) {
    ...
    const getDriverDetailDataByGroupId = async () => {
        ...
        try {
            const result = await axios.get(`http://3.24.136.21/public/getAccountOfDriverDetailByGroupId/${userData.groupCarId}`)
    ...
    const registerUser = () => {
        let Sock = new SockJS("http://3.24.136.21/public/ws")
