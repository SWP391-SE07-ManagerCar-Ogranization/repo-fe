...
import { Button, Avatar, Tooltip, Input, Typography, message } from "antd";
import { SearchOutlined } from "@ant-design/icons";
import Message from "./Message";
import avt from "../../assets/avt.jpg";
import { getAllMessageByGroupCarId } from "../../service/MessageService";
import { getAllCustomersByGroupCarId } from "../../service/CustomerService";

import "./Chatbox.module.css";
import { ChatRoomContext } from "../../context/ChatRoomContext";
import axios from "axios";

import { getAllPrivateMessagesByDriverAndCustomer } from "../../service/MessageService";
...
export default function Chatbox({ group, role }) {
  ...
  const getDriverDetailDataByGroupId = async () => {
    try {
      const result = await axios.get(
        `http://3.24.136.21:8080/public/getAccountOfDriverDetailByGroupId/${userData.groupCarId}`
      );
  ...
  const getAllPrivateMessagesByDriverAndCustomer_ = async (
    customerId,
    driverDetailId
  ) => {
    try {
      const result = await getAllPrivateMessagesByDriverAndCustomer(
        customerId,
        driverDetailId
      );
      console.log(result);
  ...
  const registerUser = () => {
    let Sock = new SockJS("http://3.24.136.21:8080/public/ws");
  ...
  const onConnected = () => {
    ...
    if (stompClient && stompClient.connected) {
      stompClient.subscribe(
        `/chatroom/${userData.groupCarId}/public`,
      ...
      if (userData.customerId && userData.driverDetailId) {
        if (userData.userId == userData.customerId) {
          stompClient.subscribe(
            `/user/${userData.driverDetailId}/private`,
            onPrivateMessageReceived
          );
        }
        if (userData.userId == userData.driverDetailId) {
          stompClient.subscribe(
            `/user/${userData.customerId}/private`,
            onPrivateMessageReceived
          );
        }
      }
  ...
  const sendPrivateMessage = () => {
    if (stompClient && stompClient.connected) {
      let chatMessage = {
        ...
        role: role,
        receiverId:
          userData.customerId == userData.senderId
            ? userData.driverDetailId
            : userData.customerId,
        message: userData.message,
        status: "MESSAGE",
    ...
    <>
      {userData.type == "PUBLIC" ? (
        <div className="h-[100vh] border-l ">
          ...
          <div
            className=" content flex flex-col p-[11px] justify-end relative"
            style={{
          ...
          >
            <div
              style={{ display: "none" }}
              className="absolute top-0 left-[30%] right-[30%] "
            >
              <Input />
              <Button>
                <SearchOutlined />
            ...
            >
              {messages?.map((message, index) => {
                const customer = customers.find(
                  (customer) =>
                    customer.accountId === message.customer ||
                    (message.customer &&
                      customer.accountId === message.customer.id)
                );
                let customerName = customer ? customer.name : "Unknown";
                let roleChat = "CUSTOMER";
                if (
                  message.driverDetail != null &&
                  (message.driverDetail.id == driverDetail.accountId ||
                    message.driverDetail == driverDetail.accountId)
                ) {
                ...
                if (
                  role == "DRIVER" &&
                  (userData?.userId == message?.driverDetail ||
                    userData?.userId == message?.driverDetail?.id)
                ) {
                  isTargetUser = true;
                  console.log(
                    `IDMESS= ${index} + ROLE = ${role} + userData?.userId= ${userData?.userId} + message?.driverDetail?.id= ${message?.driverDetail}`
                  );
                }

                if (
                  typeof message.customer == "object" &&
                ...
                return (
                  <div
                    key={index}
                    className={isTargetUser ? "flex justify-end" : ""}
                  >
                    <Message
            ...
            <div className="form flex justify-between items-center pt-1 pr-1 pb-1 mr-24 ml-24 border border-solid rounded-md bg-gradient-to-r from-gray-200 box-border to-white">
              ...
              <Button
                type="primary"
                className="font-bold"
                onClick={userData.message === "" ? null : sendPublicMessage}
              >
                Send
              </Button>
            </div>
        ...
        <div>
          private chat ={" "}
          {`${userData.groupCarId} + ${userData.customerId} + ${userData.driverDetailId}`}
          <div className="h-[100vh] border-l ">
            {/* <header className='flex justify-between h-[56px] pl-3 pr-3 items-center border-b border-black'>
                <div className='flex flex-col justify-center'>
                    <div className='m-0 font-bold'>{group.groupName}</div>
            ...
            >
              ...
              >
                {privateMessages?.map((message, index) => {
                  ...
                  if (
                    ...
                    isTargetUser = true;
                    console.log(
                      `${message.driverName} IDMESS= ${index} + ROLE = ${role} + userData?.userId= ${userData?.userId} + message?.driverDetailId= ${message?.senderId} + isTarget= ${isTargetUser}`
                    );
                  } else {
                  ...
                  if (
                    ...
                    isTargetUser = true;
                    console.log(
                      `IDMESS= ${index} + ROLE = ${role} + userData?.userId= ${userData?.userId} + message?.driverDetailId= ${message?.senderId} + isTarget= ${isTargetUser}`
                    );
                  }
              ...
              <div className="form flex justify-between items-center pt-1 pr-1 pb-1 mr-24 ml-24 border border-solid rounded-md bg-gradient-to-r from-gray-200 box-border to-white">
                ...
                <Button
                  type="primary"
                  className="font-bold"
                  onClick={userData.message === "" ? null : sendPrivateMessage}
                >
                  Send
                </Button>
              </div>
