...
import {
  getUserTransactionByCustomerAndGroupCar,
  paymentTransaction,
...
function ListGroupCar() {
  ...
  const countDown = async (id) => {
    try {
      const result = await axios.get(
        `http://3.24.136.21:8080/public/getAccountOfDriverDetailByGroupId/${id}`
      );
  ...
  const handleMembers = async (id) => {
    ...
    const listAccount = await axios.get(
      `http://3.24.136.21:8080/public/getAccountsByGroupId/${id}`
    );
  ...
  const loadGroupCar = async () => {
    try {
      const result = await axios.get(
        `http://3.24.136.21:8080/public/groupCarsByCustomerId/${userId}`
      );

      const groupCarsWithDriverDetail = await Promise.all(
        result.data.map(async (groupCar) => {
          try {
            const driverDetail = await getAccountDriverByGroupCarJoin(
              groupCar.groupId
            );
  ...
  const loadGroupCarByGroupId = async (groupId) => {
    try {
      const result = await axios.get(
        `http://3.24.136.21:8080/public/groupCarById/${groupId}`
      );
  ...
  const handleShowChatRoom = (group) => {
    ...
    const { groupId, startPoint, endPoint } = group;
  ...
  return (
    <div className="flex">
            ...
            {groupCars.map((groupCar) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  {checkDriverDetail ? (
                    <button
                      className={`flex flex-row w-[180px] font-Roboto font-bold rounded-md justify-center items-center h-[52px] ${
                        groupCar.hasDriver
      ...
      {displayChatRoom != null && (
        <ChatRoom group={displayChatRoom} role="CUTOMER" />
      )}
