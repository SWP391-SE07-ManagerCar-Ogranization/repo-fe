...
function SearchGroupCar() {
  ...
  useEffect(() => {
    ...
    try {
      if (groupCarAndUserString) {
        groupCarAndUserObject = JSON.parse(
          decodeURIComponent(groupCarAndUserString)
        );
        const { groupCar, user } = groupCarAndUserObject;
        // addOwnerTrip(user, groupCarData
        // axios.post(`http://3.24.136.21:8080/public/addCustomer/${user.id}/${groupCarData.groupId}`);
  ...
  const loadGroupCar = async () => {
    const result = await axios.get(`http://3.24.136.21:8080/public/groupCars`);
    setGroupCars(result.data);
  };
  const handleJoin = async (groupId) => {
    try {
      await axios.post(
        `http://3.24.136.21:8080/public/addCustomer/${userObject.id}/${groupId}`
      );
  ...
  const filteredGroupCars = groupCars.filter((groupCar) => {
    return (
      groupCar.startPoint.trim() === groupCarObject.startPoint.trim() ||
      groupCar.endPoint.trim() === groupCarObject.endPoint.trim()
    );
  });
