...
import {
  handleSubmit,
  handlePayment,
} from "../carpoolFunction/carpoolFunction";
...
function ListGroupCar() {
  ...
  // useEffect(()=>{
  //   try{
  //     axios.post(`http://3.24.136.21:8080/public/addCustomer/${userObject.id}/${groupCarDetail.groupId}`);
  ...
  const loadGroupCar = async () => {
    try {
      const result = await axios.get(
        `http://3.24.136.21:8080/public/groupCars`
      );
  ...
  const loadGroupCarByGroupId = async (groupId) => {
    try {
      ...
      console.error(
        `Failed to fetch group car with groupId ${groupId}:`,
        error
      );
    }
  };
  ...
  const handleJoin = async (groupId) => {
    try {
      ...
      await axios.post(
        `http://3.24.136.21:8080/public/group-car/add-customer/${userObject.accountId}/${groupId}`
      );
