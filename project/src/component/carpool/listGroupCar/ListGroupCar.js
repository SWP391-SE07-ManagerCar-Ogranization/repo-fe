...
function ListGroupCar() {
  ...
  // useEffect(()=>{
  //   try{
  //     axios.post(`http://3.24.136.21/public/addCustomer/${userObject.id}/${groupCarDetail.groupId}`);
  ...
  const loadGroupCarByGroupId = async (groupId) => {
    try {
      const result = await axios.get(`http://3.24.136.21/public/groupCarById/${groupId}`);
  ...
  const handleJoin = async (groupId) => {
    try {
      ...
      await axios.post(`http://3.24.136.21/public/group-car/add-customer/${userObject.accountId}/${groupId}`);
